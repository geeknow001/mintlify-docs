...
```bash
curl -X POST https://magic666.top/v1/video/generations \
...
resp = requests.post(
    "https://magic666.top/v1/video/generations",
    headers={
...
```javascript
const response = await fetch("https://magic666.top/v1/video/generations", {
  method: "POST",
  headers: {
    Authorization: "Bearer YOUR_API_KEY",
    "Content-Type": "application/json",
