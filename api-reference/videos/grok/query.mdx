...
```bash
curl https://magic666.top/v1/videos/video_abc123 \
...
resp = requests.get(
    "https://magic666.top/v1/videos/video_abc123",
...
```javascript
const response = await fetch("https://magic666.top/v1/videos/video_abc123", {
  headers: {
...
```bash
curl https://magic666.top/v1/videos/video_abc123/content \
  -H "Authorization: Bearer YOUR_API_KEY" \
  --output result.mp4
```
