---
title: Seedance 2.0 创建素材
description: 使用 `POST /api/asset/createMedia` 创建图片、视频或音频素材，供 Seedance 2.0 视频任务引用。
...
```bash 图片素材 cURL
curl -X POST https://magic666.top/api/asset/createMedia \
...
resp = requests.post(
    "https://magic666.top/api/asset/createMedia",
    headers={
...
const response = await fetch("https://magic666.top/api/asset/createMedia", {
  method: "POST",
  headers: {
    Authorization: "Bearer YOUR_API_KEY",
    "Content-Type": "application/json",
