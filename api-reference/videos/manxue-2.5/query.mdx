...
```bash cURL
curl -X GET https://magic666.top/v1/videos/manxue-2.5_xxx \
...
```json 200 - 已完成
{
  ...
  "seconds": "10",
  "url": "https://magic666.top/v1/videos/manxue-2.5_xxx/content",
  "video_url": "https://magic666.top/v1/videos/manxue-2.5_xxx/content",
  "metadata": {
    "cached": true,
    "content_url": "https://magic666.top/v1/videos/manxue-2.5_xxx/content",
    "expires_in": 86400,
    "cost_credits": 70
  },
  "error": null
...
```bash
curl -L https://magic666.top/v1/videos/manxue-2.5_xxx/content \
  -H "Authorization: Bearer YOUR_API_KEY" \
  --output result.mp4
```
...
- 如果任务失败，建议记录 `model`、`seconds`、`ratio`、参考图片和完整 `error`。
