...
| manxue-2.5 | `manxue-2.5` | [概览](/api-reference/videos/manxue-2.5/overview) | `/v1/videos` | 文生视频、多参考图 | multipart `model` / `prompt` / `seconds` / `ratio` / `images` |
...
| manxue-2.5 | 支持 | 可通过 `images` 表达 | 可通过 `images` 加提示词表达 | 支持最多 30 张参考图 | 模型相关 |
...
| `manxue-2.5` | manxue-2.5 | multipart 请求；`seconds` 支持 `5`、`10`、`30` 秒；`ratio` 支持 `1:1`、`3:4`、`4:3`、`9:16`、`16:9`、`21:9`；参考图通过重复的 `images` 字段传入。 |
...
4. [Wan 3.0 视频概览](/api-reference/videos/wan3/overview)
5. [Omni 视频模型概览](/api-reference/videos/omni/overview)
6. [Veo 视频概览](/api-reference/videos/veo/overview)
7. 如果你要做图生视频，先确认目标模型需要的是 `image`、`images`、`input_reference`、`referenceImages` 还是 `first_frame_image` / `last_frame_image`。
8. 如果你明确是 Kling 官方格式接入，再看 `/kling/v1/videos/*` 这一组路由。
