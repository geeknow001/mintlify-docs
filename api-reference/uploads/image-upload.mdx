---
title: 上传图片
description: 调用 `/api/upload/presign` 生成预签名上传地址，再把返回的 `public_url` 传给接受公网素材 URL 的接口。
...
```bash
curl -X POST https://magic666.top/api/upload/presign \
...
resp = requests.post(
    "https://magic666.top/api/upload/presign",
    headers={
...
```javascript
const response = await fetch("https://magic666.top/api/upload/presign", {
  method: "POST",
  headers: {
    Authorization: "Bearer YOUR_API_KEY",
    "Content-Type": "application/json",
...
3. 在接受公网素材 URL 的接口中把 `public_url` 作为输入字段传入。
