---
title: OpenAI Images 兼容图像生成
description: 使用 `POST /v1/images/generations` 调用统一图像生成入口，按 `model` 选择 `gpt-image-1`、`dall-e-3`、`dall-e-2` 等兼容模型。
...
<Info>
如果你实际使用的是 `gpt-image-2-pro`、`gpt-image-2.5-flare` 或 `gpt-image-2.5-sunburst`，还可以使用 `1:1(2K)`、`4:3(2K)`、`3:2(2K)`、`2:3(2K)`、`16:9(2K)`、`9:16(2K)`、`1:1(4K)`、`4:3(4K)`、`3:2(4K)`、`2:3(4K)`、`16:9(4K)`、`9:16(4K)` 这类高分档位；两个 `gpt-image-2.5` 模型还支持 `gpt-image-2` 的全部基础尺寸。完整映射见 [图像模型支持矩阵](/api-reference/images/model-matrix)。
</Info>

<ParamField body="quality" type="string">
图像质量。`dall-e-3` 默认补成 `standard`；`gpt-image-1` 默认补成 `auto`。`gpt-image-2.5-flare` 与 `gpt-image-2.5-sunburst` 支持 `low`、`medium`、`high`、`xhigh`、`max` 和 `auto`。
</ParamField>
