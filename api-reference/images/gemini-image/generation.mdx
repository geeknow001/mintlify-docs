...
description: 使用 `POST /v1beta/models/{model}:generateContent` 调用 Gemini 图像生成接口，统一覆盖 `gemini-3-pro-image-preview`、`gemini-2.5-flash-image-preview`、`gemini-3.1-flash-image-preview` 和 `gemini-3.1-flash-lite-image`。
...
```bash
curl -X POST https://magic666.top/v1beta/models/gemini-3-pro-image-preview:generateContent \
...
resp = requests.post(
    "https://magic666.top/v1beta/models/gemini-3-pro-image-preview:generateContent",
    headers={
