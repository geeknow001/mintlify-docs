---
title: gpt-image-2.5-sunburst 生成图像 API
description: 使用 `POST /v1/images/generations` 调用 `gpt-image-2.5-sunburst` 生成图像，支持全部尺寸档位。
api: "POST /v1/images/generations"
---

# gpt-image-2.5-sunburst 生成图像 API

`gpt-image-2.5-sunburst` 使用与 `gpt-image-2` 和 `gpt-image-2-pro` 一致的 JSON 请求格式提交图像生成请求，支持全部基础、`2K` 和 `4K` 尺寸档位。

- 请求路径为 `POST /v1/images/generations`。
- 通过 `model = "gpt-image-2.5-sunburst"` 选择目标模型。
- `prompt` 描述要生成的图像内容。
...
curl -X POST https://magic666.top/v1/images/generations \
  ...
  -d '{
    ...
    "prompt": "一张具有太阳光芒构图的未来风格产品海报，层次分明，细节丰富",
    "n": 1,
    "size": "3840x2160",
    "quality": "max",
    "response_format": "url"
...
```json 200 - URL
{
  ...
  "data": [
    {
      "url": "https://.../images/img-sunburst-abc123.png",
      "revised_prompt": "一张具有太阳光芒构图的未来风格产品海报，层次分明，细节丰富"
    }
  ]
}
```
...
异步请求仍传 `model = "gpt-image-2.5-sunburst"`，`size` 可使用本页列出的全部尺寸。提交成功后读取返回的 `id` / `task_id`，再通过 `GET /v1/images/generations/async/{taskId}` 查询状态和结果。
