...
- 当前已验证可用模型包括 `gpt-image-2`、`gpt-image-2-pro`、`gpt-image-2.5-flare`、`gpt-image-2.5-sunburst`、`gpt-image-2-vip`、Doubao Seedream 系列和 Gemini 图像模型。
...
```bash 文生图 cURL
curl -X POST https://magic666.top/v1/images/generations/async \
...
resp = requests.post(
    "https://magic666.top/v1/images/generations/async",
    headers={
...
const response = await fetch("https://magic666.top/v1/images/generations/async", {
  method: "POST",
  headers: {
    Authorization: "Bearer YOUR_API_KEY",
    "Content-Type": "application/json",
...
质量等级。通用值为 `low`、`medium`、`high`、`auto`；`gpt-image-2.5-flare` 与 `gpt-image-2.5-sunburst` 还支持 `xhigh` 和 `max`。未传时默认 `auto`。
</ParamField>
...
任务使用的图像模型，例如 `gpt-image-2`、`gpt-image-2-pro`、`gpt-image-2.5-flare`、`gpt-image-2.5-sunburst`、`gpt-image-2-vip`、`doubao-seedream-4-0-250828`、`gemini-3-pro-image-preview` 或 `gemini-3.1-flash-lite-image`。
</ResponseField>
...
最终结果对象的输出尺寸或清晰度，例如 `1024x1024`、`1536x1024`、`3840x2160` 或 Gemini 的 `2K`。
</ResponseField>

<ResponseField name="quality" type="string">
最终结果对象的质量等级，例如 `low`；`gpt-image-2.5-flare` 与 `gpt-image-2.5-sunburst` 也可能返回 `xhigh` 或 `max`。
</ResponseField>
...
- `gpt-image-2.5-flare` 与 `gpt-image-2.5-sunburst` 支持全部基础、`2K` 和 `4K` 尺寸，具体 `WxH` 映射见各自生成页面。
- `gpt-image-2.5-flare` 与 `gpt-image-2.5-sunburst` 的 `quality` 还支持 `xhigh` 和 `max`。
- `gpt-image-2-vip` 的 `size` 必须满足：每条边是 `16px` 倍数，最长边不超过 `3840px`，长短边比例不超过 `3:1`，总像素数在 `655,360` 到 `8,294,400` 之间。
- Gemini 图像模型的 `size` 表示清晰度：`gemini-3-pro-image-preview` 可传 `1K` 或 `2K`；`gemini-2.5-flash-image-preview`、`gemini-3.1-flash-image-preview` 与 `gemini-3.1-flash-lite-image` 会按 `1K` 处理。
- Gemini 异步生图的 `aspect_ratio` 传入什么比例就按什么比例生成，常见值包括 `1:1`、`16:9`、`9:16`、`4:3`、`3:4`、`3:2`、`2:3`、`21:9`。
...
- [gpt-image-2.5-flare 生成图像 API](/api-reference/images/gpt-image-2.5-flare/generation)
