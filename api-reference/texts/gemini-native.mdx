---
title: Gemini 原生格式
description: 使用 Google Gemini 原生路径与请求体调用 generateContent、streamGenerateContent 与模型查询。
...
```bash
curl -X POST https://magic666.top/v1beta/models/gemini-3.5-flash:generateContent \
...
```bash
curl -N -X POST https://magic666.top/v1beta/models/gemini-3.5-flash:streamGenerateContent \
  -H "x-goog-api-key: YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "contents": [
