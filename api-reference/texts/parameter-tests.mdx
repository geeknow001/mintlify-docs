...
本页记录一组小请求测试，用来验证当前 API Key、模型和参数组合是否可用。基础参数测试时间为 2026-07-07，图像输入补测时间为 2026-07-12；公开文档示例统一使用 `https://magic666.top`。模型和渠道会变化，生产环境请用自己的 API Key 重新跑一遍。
...
```bash
curl -X POST https://magic666.top/v1/chat/completions \
...
```bash
curl -X POST https://magic666.top/v1/responses \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "gpt-5.5",
...
```bash
curl -X POST https://magic666.top/v1/messages \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
